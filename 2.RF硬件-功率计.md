...
```mermaid
sequenceDiagram
    participant DUT as DUT (发射机/UE/模块)
    participant PM as 功率计
    participant SA as 频谱/信号分析仪
    participant WT as 无线综合测试仪

    DUT->>PM: 发射 CW 或调制信号
    Note right of PM: 测量平均功率<br/>（Thermal / Avg Power Sensor）

    DUT->>PM: 脉冲或调制信号
    Note right of PM: 峰值检测<br/>（Peak Power Sensor → 得到 Peak Power）

    DUT->>SA: 发射调制信号
    Note right of SA: Channel Power 功能 → 平均功率<br/>Zero Span 捕捉 → 峰值功率<br/>结合 CCDF → PAPR

    DUT->>WT: 在协议模式下发射
    Note right of WT: 自动给出<br/>最大功率 / 最小功率 / 平均功率 / 峰值功率
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjM2OTcwODg2XX0=
-->