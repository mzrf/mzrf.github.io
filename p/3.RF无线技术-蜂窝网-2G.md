...
```mermaid
flowchart LR
    Fuser["前向链路：多用户业务信道"] --> Fw["沃尔什正交码"]
    Fpilot["前向链路：导频信道"] --> Fcomb["码片级叠加与扩频"]
    Fsync["前向链路：同步/寻呼等控制信道"] --> Fcomb
    Fw --> Fcomb
    Fcomb --> Ftx["宽带信号发射（下行）"]

    Ftx --> Rrx["空口：同频段，不同扩频码"]
    Rrx --> Rtx["宽带信号发射（上行）"]

    Ruser["反向链路：用户数据"] --> Rpn["长PN/短PN扩频"]
    Rpn --> Rtx
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc0NDQ0Njk1LC0xMTY0ODUzNDAwXX0=
-->