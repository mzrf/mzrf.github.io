...
> 注：在教材中列出**精确数值与条文**时，建议对应标准条款号，随版本更新维护。

----------

#### 1.6.14 “从 2G 到 3G”的技术启示（承上启下）
...
### 附：两个配套小图（可放入 1.8 示意图小节）

**（A）上下行 3 时隙错位示意（“双向”明确）**

`flowchart TB
  subgraph "下行帧（BTS→MS）"
    DL1["'TS1'"] --> DL2["'TS2'"] --> DL3["'TS3'"] --> DL4["'TS4'"] --> DL5["'TS5'"] --> DL6["'TS6'"] --> DL7["'TS7'"] --> DL8["'TS8'"]
  end
  subgraph "上行帧（MS→BTS，延后3个时隙）"
    UL1["'TS1+3'"] --> UL2["'TS2+3'"] --> UL3["'TS3+3'"] --> UL4["'TS4+3'"] --> UL5["'TS5+3'"] --> UL6["'TS6+3'"] --> UL7["'TS7+3'"] --> UL8["'TS8+3'"]
  end` 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQzNzY3NzY3NCwtNzQ0NDQ2OTUsLTExNj
Q4NTM0MDBdfQ==
-->